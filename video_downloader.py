...
def request_video(url, referer='', timeout=30, retry_count=3):
    # Use a modern User-Agent to avoid rejection by servers
    ...
    for attempt in range(1, retry_count + 1):
        try:
            logging.info('Requesting {} (attempt {}/{})'.format(url, attempt, retry_count))
            response = urllib.request.urlopen(request, timeout=timeout, context=ssl_context)
            
            # Check HTTP status code
            if response.status != 200:
                logging.warning('HTTP {} for {}'.format(response.status, url))
                continue
            ...
            if len(data) == 0:
                logging.warning('Empty response for {}'.format(url))
            ...
            logging.error('URL Error: {}'.format(e.reason))
            if attempt < retry_count:
                time.sleep(random.uniform(2.0, 4.0))
            else:
                raise
    ...
    raise Exception('Failed to download {} after {} attempts'.format(url, retry_count))
...
def download_aslpro(url, dirname, video_id):
    ...
    try:
        data = request_video(url, referer='https://www.aslpro.com/')
        save_video(data, saveto)
        logging.info('Successfully downloaded aslpro video {}'.format(video_id))
    except Exception as e:
        logging.error('Failed to download aslpro video {}: {}'.format(video_id, str(e)))
...
def download_others(url, dirname, video_id):
    ...
    try:
        ...
        save_video(data, saveto)
        logging.info('Successfully downloaded video {}'.format(video_id))
    except Exception as e:
        logging.error('Failed to download video {}: {}'.format(video_id, str(e)))
